...
async def handle_help_command(event):
    ...
    help_text = (
        ...
        "👁 **Multimodal Rasm Tahlili (Vision AI):**\n"
        "• `.see <savol>` (rasmga reply) — Rasm mazmunini, ob'ektlarni tahlil qilish yoki masalani yechish\n"
        "• `.ocr` (rasmga reply) — Rasm yoki skrinshotdagi barcha matnlarni ko'chirib olish\n\n"
        "📥 **Media Yuklovchi:**\n"
        "• `.dl <havola>` (yoki reply) — Instagram Reels, TikTok (suv belgisiz), YouTube Shorts yuklab berish\n\n"
        "✍️ **Matn Tahrirchisi & Grammatika:**\n"
        "• `.fix` (matnga reply) — Imlo, grammatika va punktuatsiya xatolarini tuzatish\n"
        "• `.formal` (matnga reply) — Matnni rasmiy biznes uslubiga aylantirish\n"
        "• `.informal` / `.shaxsiy` — Do'stona, samimiy uslubga o'girish\n\n"
        "🧹 **Chatni Tozalash:**\n"
        "• `.del` (xabarga reply) — Xabarni darhol o'chirish\n"
        "• `.purge <soni>` — Chatdagi so'nggi N ta xabarni tozalash (masalan `.purge 20`)\n\n"
        "💵 **Valyuta & Ob-havo:**\n"
        "• `.kurs` / `.valyuta` — Markaziy Bankning bugungi rasmiy kurslari\n"
        "• `.weather [shahar]` — Toshkent yoki ko'rsatilgan shahar ob-havosi\n\n"
        "🗣 **Ovozli Xizmatlar (Voice):**\n"
        "• `.voice <matn>` (yoki reply) — Matnni inson ovozidagi xabar (Voice Note) ga aylantirish\n"
        "• `.text` (ovozga reply) — Ovozli xabarni matnga o'girish\n\n"
        "📝 **Chat Xulosasi & Eslatmalar:**\n"
        "• `.summary [soni]` — Chatdagi so'nggi xabarlarni xulosalash\n"
        "• `.remind <vaqt> <matn>` — Aqlli eslatma o'rnatish\n"
        "• `.reminders` — Faol eslatmalar ro'yxati\n"
        "• `.delremind <ID>` — Eslatmani bekor qilish\n\n"
        "🔗 **QR-Kod va Stiker:**\n"
        "• `.qr <matn>` / `.qr` (rasmga) — QR yaratish yoki o'qish\n"
        "• `.sticker` (rasmga) — Rasmni stikerga aylantirish\n"
        "• `.unsticker` (stikerga) — Stikerni rasmga aylantirish\n\n"
        "🔍 **Qidiruv:**\n"
        "• `.google <so'rov>` — Google/DuckDuckGo qidiruv\n"
        "• `.wiki <so'rov>` — Wikipedia'dan ma'lumot qidirish\n\n"
        "👤 **Profil Boshqaruvi:**\n"
        "• `.bio <matn>` — Profil bio (tarjimayi hol) ni o'zgartirish\n"
        "• `.name <ism>` — Profil ismini o'zgartirish\n"
        "• `.photo` (rasmga) — Profil rasmini o'zgartirish\n\n"
        "📂 **Fayl Konvertori & Boshqalar:**\n"
        "• `.topdf` (rasmga) — Rasmni PDF ga aylantirish\n"
        "• `.tomp3` (videoga) — Videodan MP3 ajratib olish\n"
        "• `.seen <user>` — Foydalanuvchining onlayn holati\n"
        "• `.spy` — Kanaldagi a'zolar, admin log va reaksiyalar tahlili\n"
        "• `.spy <N>` — Oxirgi N ta xabar va reaksiyalar tahlili\n"
        "• `.spy watch <user/contacts>` — Gumondorlarni korrelyatsiyaga kiritish\n"
        "• `.spy rank` — Gumondorlar moslik reytingi (% hisobida)\n"
        "• `.spy hunt` (yoki `.spy monitor`) — Timing attack (vaqt korrelyatsiyasi)\n"
        "• `.spy check <user>` — Gumondor shaxsning kanaldagi izlarini tekshirish\n"
        "• `.spy trap` — Obunasiz kuzatuvchilarni fosh qiluvchi tuzoq qo'llanmasi\n"
        "• `.spy stop` — Kuzatuvni to'xtatish\n\n"
        "🌐 **Boshqa Buyruqlar:**\n"
        "• `.tr <matn>` (yoki reply) — Matnni o'zbek tiliga tarjima qilish\n"
        "• `.ai <savol>` — Gemini AI ga tezkor savol berish\n"
        "• `.calc <ifoda>` — Xavfsiz kalkulyator\n"
        "• `.info` — Tizim holati va statistika\n"
